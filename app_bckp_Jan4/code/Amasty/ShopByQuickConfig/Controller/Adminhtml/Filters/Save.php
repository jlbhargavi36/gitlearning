...
use Amasty\ShopByQuickConfig\Ui\DataProvider;
...
class Save extends Action implements HttpPostActionInterface
{
    ...
    {
        $bothPositionItems = [];
        foreach ([DataProvider::KEY_SIDE_ITEMS, DataProvider::KEY_TOP_ITEMS] as $blockKey) {
            foreach ($this->getRequest()->getParam($blockKey, []) as $itemData) {
                ...
                if ($item->getBlockPosition() === FilterPlacedBlock::POSITION_BOTH) {
                    switch ($blockKey) {
                        case DataProvider::KEY_SIDE_ITEMS:
                            $item->setSidePosition($position);
                            break;
                        case DataProvider::KEY_TOP_ITEMS:
