...
class PreCoupon implements ObserverInterface
{
	...
	$controller = $observer->getControllerAction();
        $couponCode = $controller->getRequest()->getParams();
	$logger->info("couponCode : ". json_encode($couponCode));
	if(isset($couponCode['coupon_code'])){
		$logger->info("Coupon : ". json_encode($couponCode));
		$ruleId =  $this->couponModel->loadByCode($couponCode['coupon_code'])->getRuleId();
	    	//$rule = $this->ruleRepository->getById($ruleId);
	    	$logger->info("ruleId : ". json_encode($ruleId));
		$couponId = $this->dataHelper->getEmpCouponId();
		if($ruleId == $couponId && $this->customerSession->isLoggedIn()){
